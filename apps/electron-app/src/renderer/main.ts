...
(async () => {
  try {
    ...
    Promise.all([
      ...
      // Now set up the listener for navigation events
      window.electronAPI.onNavigate((_event, route) => {
        // `route` is the string sent from main process, e.g. "privatize"
        // Navigate to the corresponding route in your router
        // For example, if your routes look like "/privatize", "/prompt-manager", etc.:
        router.push(`/${route}`);
      });
